...
- [UploadExceptionHandler.java](#uploadexceptionhandlerjava)
...
### `readScaledImageForCompression(byte[] imageBytes, int maxSide)` _(private)_
...
### `prepareImageForPayloadLimit(byte[] imageBytes, String filename)` _(private)_
...
### `readImageDimensions(byte[] imageBytes)` _(private)_
...
### `toCompressedFilename(String filename)` _(private)_
...
### `convertToRGB(BufferedImage img)` _(private)_
...
### `isPayloadTooLargeError(Exception exc)` _(private)_
...
### `detectImageContentType(String filename)` _(private)_
...
### `extractDetections(Map<String, Object> payload)` _(private)_
...
### `getDetectionLabel(Map<String, Object> det)` _(private)_
...
### `getDetectionScore(Map<String, Object> det)` _(private)_
...
### `weatherCodeToString(int code)` _(private)_
...
### `buildMetadataFromResultSet(ResultSet rs)` _(private)_
...
Polls a Gmail inbox for unread emails containing image attachments, runs each image through the upload pipeline, and replies to the sender with elk count results. Email polling is triggered manually through the task endpoint; startup polling is disabled.

---

### `buildGmailService()` _(private)_
...
### `resolveWritableTokenStoreDir(String tokenPath)` _(private)_

| Field             | Detail                                                                                                                                                                                                                                                                   |
| ----------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| **Inputs**        | `tokenPath` (String) — configured Gmail token path from `SecretConfig`                                                                                                                                                                                                   |
| **Outputs**       | `File` — writable token store directory, or a fallback directory under `/tmp`                                                                                                                                                                                            |
| **Functionality** | Resolves the runtime token datastore location. Prefers the configured directory when it is writable, but automatically falls back to a writable `/tmp` directory when the configured path points at a read-only Cloud Run secret mount or another non-writable location. |
| **Dependencies**  | `isDirectoryWritable`, `prepareFallbackTokenStore`                                                                                                                                                                                                                       |
| **Called by**     | `buildGmailService`                                                                                                                                                                                                                                                      |

---

### `isDirectoryWritable(File directory)` _(private)_

| Field             | Detail                                                                                                                               |
| ----------------- | ------------------------------------------------------------------------------------------------------------------------------------ |
| **Inputs**        | `directory` (File) — directory to test for write access                                                                              |
| **Outputs**       | `boolean` — `true` if a temporary file can be created and deleted in the directory                                                   |
| **Functionality** | Performs a real write probe instead of relying on `canWrite()`, which can be misleading on mounted or container-managed filesystems. |
| **Dependencies**  | `java.nio.file.Files`                                                                                                                |
| **Called by**     | `resolveWritableTokenStoreDir`, `prepareFallbackTokenStore`                                                                          |

---

### `prepareFallbackTokenStore(File tokenPathFile, File configuredTokenDir)` _(private)_

| Field             | Detail                                                                                                                                                      |
| ----------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Inputs**        | `tokenPathFile` (File) — the configured token path or StoredCredential file; `configuredTokenDir` (File) — directory derived from the configured token path |
| **Outputs**       | `File` — writable fallback directory under `/tmp` containing a copied `StoredCredential`                                                                    |
| **Functionality** | Creates and/or reuses a writable token datastore under `/tmp` and seeds it from the configured credential file when available.                              |
...
| **Called by**     | `EventScheduler.runEmailPollingJob`, `TaskController.runEmailPollingTask`                                                                                                                                                                                 |

---

### `markAsRead(Gmail gmail, String user, String messageId)` _(private)_
...
### `sendReply(Gmail gmail, String user, String toEmail, String originalSubject, String bodyText)` _(private)_
...
### `resolveAttachmentDisplayName(String filename, String ext)` _(private)_

| Field             | Detail                                                                                                                          |
| ----------------- | ------------------------------------------------------------------------------------------------------------------------------- |
| **Inputs**        | `filename` (String) — original attachment filename, may be blank; `ext` (String) — fallback extension determined from MIME type |
| **Outputs**       | `String` — display name used in replies and logs                                                                                |
| **Functionality** | Returns the original filename when available, otherwise generates a stable fallback name for email attachments.                 |
| **Dependencies**  | None                                                                                                                            |
| **Called by**     | `pollAndProcess`                                                                                                                |

---

### `formatElkCount(Integer elkCount)` _(private)_

| Field             | Detail                                                                      |
| ----------------- | --------------------------------------------------------------------------- |
| **Inputs**        | `elkCount` (Integer) — detected elk count, may be `null`                    |
| **Outputs**       | `String` — count as text, or `"0"` when unknown/null                        |
| **Functionality** | Formats elk counts for status messages in a consistent reply-friendly form. |
| **Dependencies**  | None                                                                        |
| **Called by**     | `pollAndProcess`                                                            |

---

### `extractHeader(Message message, String headerName)` _(private)_
...
### `extractEmailAddress(String headerValue)` _(private)_
...
### `collectImageAttachmentParts(MessagePart part, List<MessagePart> imageParts, boolean attachmentsOnly)` _(private)_
...
### `isAllowedMimeType(String mimeType)` _(private)_
...
| **Called by**     | `TaskController.runEmailPollingTask` (via HTTP POST `/internal/tasks/poll-email`)                                                                                |
...
### `PythonInferenceClient(RestClient.Builder builder)` _(private inner constructor)_
...
### `inferCount(byte[] imageBytes)` _(private inner)_
...
### `inferCounts(List<ImagePayload> images)` _(private inner)_
...
### `validateUploadedFiles(MultipartFile[] files)` _(private)_

| Field             | Detail                                                                                                                                                                             |
| ----------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Inputs**        | `files` (MultipartFile[]) — array of uploaded files to validate                                                                                                                    |
| **Outputs**       | void — throws `IllegalArgumentException` if any file is missing, too large, over the per-request file count, or has a disallowed extension                                         |
| **Functionality** | Ensures at least one file is present, enforces the 10-file and 10MB upload limits, and verifies that every file has an allowed image extension (`.png`, `.jpg`, `.jpeg`, `.heic`). |
...
Global exception handler for multipart upload failures.

---

### `handleMaxUploadSizeExceeded(MaxUploadSizeExceededException e)`

| Field             | Detail                                                                                         |
| ----------------- | ---------------------------------------------------------------------------------------------- |
| **Inputs**        | `e` (MaxUploadSizeExceededException) — multipart upload exception from Spring                  |
| **Outputs**       | `ResponseEntity<?>` — 400 response containing a user-facing upload size message                |
| **Functionality** | Converts oversized multipart request exceptions into a consistent JSON error for the frontend. |
| **Dependencies**  | `org.springframework.web.multipart.MaxUploadSizeExceededException`                             |
...
| **Outputs**       | `ResponseEntity<?>` — 400 response containing a user-facing multipart error message               |
| **Functionality** | Converts malformed or oversized multipart requests into a consistent JSON error for the frontend. |
| **Dependencies**  | `org.springframework.web.multipart.MultipartException`                                            |
| **Called by**     | Spring MVC exception handling                                                                     |

---

### `parseUploadMetadata(MultipartFile[] files, String metadataJson)` _(private)_
...
### `buildMetadataForUpload(Path tempFile, String originalName, UploadMetadataData uploadData, int fileIndex)` _(private)_
...
### `isAllowedImageName(String filename)` _(private)_
...
### `normalizedStorageExtension(String ext)` _(private)_
...
### `addMetadataToFileInfo(Map<String, Object> fileInfo, Metadata meta)` _(private)_
...
### `downloadFromCloudUri(String cloudUri, Path destinationPath)` _(private)_
...
### `removeExtension(String filename)` _(private)_
...
### `extractExifFromPng(byte[] pngBytes)` _(private)_
...
### `parseIFD(ByteBuffer bb, int offset, ExifData d)` _(private)_
...
### `parseExifIFD(ByteBuffer bb, int offset, ExifData d)` _(private)_
...
### `charRef(ByteBuffer bb, int type, int count, int value)` _(private)_
...
### `parseGPSIFD(ByteBuffer bb, int offset, ExifData d)` _(private)_
...
### `readRationalTriplet(ByteBuffer bb, int offset)` _(private)_
...
### `readRational(ByteBuffer bb, int offset)` _(private)_
...
### `bytesToHex(byte[] bytes)` _(private)_
...
### `insertExifIntoJpeg(byte[] jpegBytes, byte[] exifBytes)` _(private)_
...
### `ensureExifHeader(byte[] exifBytes)` _(private)_
...
### `readIntBE(byte[] buf, int offset)` _(private)_
...
### `isMetadataEmpty(Metadata meta)` _(private)_
...
### `parsePngMetadata(String filename)` _(private)_
...
### `resolveFileByName(String filename)` _(private)_
...
### `doubleEquals(Double a, Double b, double eps)` _(private)_
...
### `isAllowedImageType(String contentType)` _(private)_
...
### `normalize(String value)` _(private)_
...
Spring REST controller that manages the token-protected email polling endpoint used by external schedulers.
